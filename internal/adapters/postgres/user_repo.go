...
RETURNING id::text;`
	...
	if err := r.db.Pool.QueryRow(ctx, q,
		tenantID, email, username, passHash, attrsJSON,
	).Scan(&id); err != nil {
		return "", err
	}
...
func (r *UserRepo) GetUserByIdentifier(
	ctx context.Context,
	tenantID, identifier string,
) (userID, passwordHash string, email, username *string, err error) {
	const q = `
...
LIMIT 1;`
	...
	var em, un *string
	if err = r.db.Pool.QueryRow(ctx, q, tenantID, identifier).Scan(
		&userID, &passwordHash, &em, &un,
	); err != nil {
...
func (r *UserRepo) GetUserByID(ctx context.Context, tenantID, userID string) (email *string, username *string, err error) {
	const q = `
...
LIMIT 1;`
	...
	if err = r.db.Pool.QueryRow(ctx, q, tenantID, userID).Scan(&em, &un); err != nil {
		return nil, nil, errors.New("not_found")
	}
